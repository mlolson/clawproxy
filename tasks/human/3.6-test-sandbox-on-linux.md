...
Manually test that the sandbox prevents access to secrets on Linux
...
```bash
# Setup
mkdir -p ~/.clawproxy/secrets
echo "super-secret-key" > ~/.clawproxy/secrets/test
chmod 600 ~/.clawproxy/secrets/test

# Without sandbox (should work)
cat ~/.clawproxy/secrets/test
# Output: super-secret-key

# With sandbox (should fail)
clawproxy-run cat ~/.clawproxy/secrets/test
...
cargo run --bin clawproxy-run -- -c "env | grep -i proxy"
# Expected:
...
cargo run --bin clawproxy-run -- -c "ls /tmp"
cargo run --bin clawproxy-run -- -c \"echo "hello world\"
cargo run --bin clawproxy-run -- -c "python3 -c \"print('works')\""
# All should succeed
```
...
cargo run --bin clawproxy-run -- -c "curl -I https://example.com"
